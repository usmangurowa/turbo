-- Upgrades a pre-2026-06 Better Auth "apikey" table (user_id → reference_id).
-- Skipped entirely when the legacy column is absent (fresh databases get the
-- final shape from 0000_baseline_auth_schema).
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'apikey' AND column_name = 'user_id'
  ) THEN
