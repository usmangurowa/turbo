...
**Is an environment complete?** `pnpm ai:env:infisical --env prod` compares the key names in an Infisical environment with the contract table and lists the required variables it lacks (the process would refuse to boot), the optional ones it lacks (those features are off), any keys the contract does not know (typos, leftovers), and the `NEXT_PUBLIC_*`/`EXPO_PUBLIC_*` keys it holds (build-time, not secret). `--all` walks `dev`, `staging`, and `prod` (`--envs a,b` for other slugs); `--json` prints an object. Run it before a deploy and after adding a variable to `.env.example`. Only names are read — the script never prints or writes a value. `pnpm ai:env:infisical:strict` exits 1 on missing required variables and 2 when Infisical cannot be read (no CLI, no credentials, offline), so a pipeline without credentials can tell "skipped" from "broken". It uses the same credential order as the wrapper above and is not a CI step: CI has no Infisical credentials (`.ai/specs/active/infisical-env-drift.spec.md`).
